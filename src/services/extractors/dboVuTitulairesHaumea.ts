...
import { getDboVuTitulairesHaumeaQuery } from '../../db/queries/source.js';
import { insertDboVuTitulairesHaumeaQuery } from '../../db/queries/target.js';
import { truncateTable } from '../truncateTable.js';

export class DboVuTitulairesHaumeaExtractor {
  ...
  async extract(): Promise<void> {
    try {
      ...
      logger.info('Vidage de la table dbo_vutitulaires_haumea...');
      await truncateTable(this.targetPool, 'dbo_vutitulaires_haumea');
      logger.info('Table dbo_vutitulaires_haumea videe avec succes');

      // 2. Executer la requête source
      logger.info('Extraction des donnees de VU titulaires...');
      const sourceQuery = getDboVuTitulairesHaumeaQuery();
      ...
      if (rows.length === 0) {
        logger.warn('Aucune donnee trouvee pour les VU titulaires');
        return;
      }

      logger.info(`${rows.length} VU titulaires trouves`);

      // 3. Inserer les donnees dans la table cible
      logger.info('Insertion des donnees dans dbo_vutitulaires_haumea...');
      const insertQuery = insertDboVuTitulairesHaumeaQuery();
      let insertedCount = 0;
      let errorCount = 0;
      const logFrequency = parseInt(process.env.NB_LIGNES_DEBUG_DBOVUTITULAIRESHAUMEA || '1000');
      ...
      logger.error('Erreur lors de l\'extraction des VU titulaires:', error);
      throw error;
    }
  }
} 